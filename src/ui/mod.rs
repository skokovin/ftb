...
pub fn re_load_mesh(
    ...
    mut query_meshes: Query<(Entity, &MeshPipe)>,
    mut query_centerlines: Query<(Entity, &PipeCenterLine)>,
    query_transform_machine: &mut Query<(&mut Transform, Option<&MachinePart>, Option<&Name>)>,
) {
    for (entity, _) in &mut query_meshes {
        commands.entity(entity).despawn();
    }
    ...
    /*query_transform_machine.iter_mut().for_each(|(mut transform, mesh_machine_part, name)| {
        let  (l,d)=LRACLR::total_len_out_d(&bend_commands.straight);
            ...
            Some(_) => {
                match name {
                    ...
                    Some(n) => {
                        match n.as_str() {
                            "pens"=>{
                                transform.translation.x = transform.translation.x - l as f32;
                            }
                            &_ => {}
                        }
                    }
